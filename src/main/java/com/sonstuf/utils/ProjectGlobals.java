package com.sonstuf.utils;

/**
 * Created by enrico on 24/11/15.
 */
public class ProjectGlobals {
	public static final String DATE_INPUT_FORMAT = "yyyy-MM-dd";
	public static final String DATE_OUTPUT_FORMAT = "dd/MM/yyyy";
	public static final String DATETIME_OUTPUT_FORMAT = "dd/MM/yyyy HH:mm";
	
	/**
	 * Path of the dafault error page
	 */
	public static final String ERROR_PAGE = "/webpages/errorPage.jsp";
}
